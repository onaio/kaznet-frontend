...
describe('store/tasks/actions', () => {
    ...
    it('should fetch tasks from server', async () => {
        ...
        expect(dispatches[0].getAction()).toEqual({ type: actionTypes.TASKS_FETCHED, tasksById: fixtures.tasksById});
    });

    it('should fetch tasks and print to console on error', async () => {
        TaskService.getTaskList.mockImplementationOnce(() => { throw new Error('oops'); });
